			...
			body: new Container(
				...
				child: new ListView(
					children: <Widget>[
						...
						new Text("5 day / 3 Hour Forecast", style: new TextStyle(fontSize: 20.0, color: Colors.white),),
						...
						child: new Column(
							...
							children: <Widget>[
								...
								new Padding(
									...
									child: new Text("${content['$jsonObject']['$jsonField'].toString()} $suffix", style: new TextStyle(fontSize: 25.0, color: Colors.white),),
								),
		...
		return FutureBuilder(
			...
			builder: (BuildContext context, AsyncSnapshot<Map> snapshot){
				if(snapshot.hasData){
					...
					DateTime date = DateTime.fromMillisecondsSinceEpoch(content['sys']['$riseOrSet'] * 1000);
					var _date = dateFormat.format(date);

					return new Column(
						children: <Widget>[

							new Image.asset(riseOrSet == "sunrise" ? "assets/images/sunrise.png" : "assets/images/sunset.png" , height: 50.0, width: 50.0,),
							new Text("$_date", style: new TextStyle(color: Colors.white),),
						],
					);
				}else{
					return new Container();
					...
					return new Container(
						...
						child: ListView.builder(
							...
							itemBuilder: (BuildContext context, position) {
								...
								DateTime date = DateTime.fromMillisecondsSinceEpoch(_days[index]['dt'] * 1000);
								DateTime time = DateTime.fromMillisecondsSinceEpoch(_days[index]['dt'] * 1000);
								...
								return new Padding(
									...
									child: new Column(
										...
										children: <Widget>[
											...
											new Text("$_date", style: new TextStyle(color: Colors.white),),
											new Text("$_time", style: new TextStyle(color: Colors.white),),
											new Padding(padding: EdgeInsets.only(top: 10.0)),

											new Image.asset("assets/images/${_days[index]['weather'][0]['main'].toString()}.png", height: 40.0, width: 40.0,),
											new Text("${temp.round()} ºC", style: new TextStyle(color: Colors.white, fontSize: 18.0),),
											new Text("${_days[index]['main']['humidity']} %", style: new TextStyle(color: Colors.white, fontSize: 15.0),),
